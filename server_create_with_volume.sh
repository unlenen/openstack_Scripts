...
function create_instances(){
	...
	openstack server create --flavor ${FLAVOR_NFS} --volume $VOLUME_ID  --nic net-id=$NETWORK_ID --key-name=cloud --security-group $SEC_GROUP_ID --user-data ./inst-script/openstack-nfs-server.sh onap-nfs
	...
	for a in {1..3}; do 
		...
		openstack server create --flavor ${FLAVOR_CONTROLLER} --volume $VOLUME_ID  --nic net-id=$NETWORK_ID --key-name=cloud --security-group $SEC_GROUP_ID --user-data ./inst-script/openstack-k8s-controlnode_new.sh  onap-control-$a
...
#		openstack server create --flavor ${FLAVOR_CONTROLLER} --image $IMAGE_ID  --nic net-id=$NETWORK_ID --key-name=cloud --security-group $SEC_GROUP_ID --user-data ./inst-script/openstack-k8s-controlnode_new.sh  onap-control-$a
	...
	for a in {1..12}; do 
		...
		openstack server create --flavor ${FLAVOR_WORKER} --volume $VOLUME_ID  --nic net-id=$NETWORK_ID --key-name=cloud --security-group $SEC_GROUP_ID --user-data ./inst-script/openstack-k8s-workernode_new.sh  onap-k8s-$a
